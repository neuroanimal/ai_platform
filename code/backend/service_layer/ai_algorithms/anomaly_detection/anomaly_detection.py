...
@dataclass
class AnomalyResult:
    """Result of anomaly detection containing scores, anomalies, and threshold"""
...
class ChangePointResult:
    """Result of change point detection containing change points and scores"""
...
class AnomalyDetection:
    ...
    def isolation_forest(
        self, data: np.ndarray, contamination: float = 0.1, **kwargs
    ) -> AnomalyResult:
        ...
        anomalies = np.nonzero(anomaly_labels == -1)[0]
    ...
    def one_class_svm(
        self, data: np.ndarray, nu: float = 0.1, kernel: str = "rbf", **kwargs
    ) -> AnomalyResult:
        ...
        return AnomalyResult(
            anomaly_scores=anomaly_scores, anomalies=anomalies, threshold=0.0
        )

    def spc_control_chart(
        self, data: np.ndarray, window_size: int = 30, sigma_level: float = 3.0
    ) -> AnomalyResult:
        ...
        rolling_mean = np.concatenate(
            [np.full(window_size - 1, rolling_mean[0]), rolling_mean]
        )
        ...
        # Detect anomalies
        anomalies = np.nonzero((data > ucl) | (data < lcl))[0]
    ...
    def bayesian_changepoint(
        self, data: np.ndarray, model: str = "rbf", min_size: int = 2, **kwargs
    ) -> ChangePointResult:
        ...
        return ChangePointResult(change_points=np.array(change_points))
    ...
    def statistical_anomaly(
        ...
        if method == "zscore":
            ...
            anomalies = np.nonzero(anomaly_scores > threshold)[0]

        elif method == "modified_zscore":
            median = np.median(data)
            mad = np.median(np.abs(data - median))
            anomaly_scores = 0.6745 * (data - median) / mad
            anomalies = np.nonzero(np.abs(anomaly_scores) > threshold)[0]
            ...
            anomalies = np.nonzero((data < lower_bound) | (data > upper_bound))[0]

        else:
            raise ValueError(f"Unknown method: {method}")

        return AnomalyResult(
            anomaly_scores=anomaly_scores, anomalies=anomalies, threshold=threshold
        )
    ...
    def ensemble_anomaly(self, data: np.ndarray, methods: list = None) -> AnomalyResult:
        ...
        for method in methods:
            try:
                ...
                normalized_scores = (
                    result.anomaly_scores - result.anomaly_scores.min()
        ...
        ensemble_anomalies = np.nonzero(anomaly_votes > threshold_votes)[0]
