...
def fix_values(yaml_content, map_path_flat, map_path_helm, system_size, priorities):
    row_num = 0
    output = ""
            ...
            ### if row_num > 1:
                ...
                ### for i in range(row_num - 1, row_num - 2, -1):
                    ...
                    if in_rows[i].lstrip().startswith('#'):
                        ...
                        # eg.: /rerg/frfr/foirnfJnrnn/rfrf starts with /rerg/frfr
                        if VERBOSE:
    ...
    for path in value_of_path.keys():
        ...
        if value is None:
            ...
            if value.strip().startswith("{{"):
                ...
                default_from_flat = None
                defaults_from_flat = None
                example_from_flat = None
                recomen_from_flat = None
                if path in map_path_flat:
                    all_from_flat = map_path_flat[path]
                    default_from_flat = all_from_flat["default"] if "default" in all_from_flat else None
                    for _key in list(all_from_flat.keys()):
                        if isinstance(_key, bool):
                            ...
                            if defaults_from_flat is None:
                                defaults_from_flat = {}
                            defaults_from_flat[_key] = all_from_flat[_key]
                    example_from_flat = all_from_flat["example"] if "example" in all_from_flat else None
                    recomen_from_flat = all_from_flat["recommended_value"] if "recommended_value" in all_from_flat else None
                else:
                    print(f"Warning: Cannot find path {path} in FLAT parameters, please check it manually!")
                ...
                for priority in priorities:
                    if priority == "FLAT.recommended_value":
                        print(f"\t{priority}: {recomen_from_flat}")
                        if not taken and recomen_from_flat is not None:
                            taken = True
                            if isinstance(recomen_from_flat, bool):
                                if recomen_from_flat:
                                    recomen_from_flat = "true"
                                else:
                                    recomen_from_flat = "false"
                            elif isinstance(recomen_from_flat, int):
                                recomen_from_flat = str(recomen_from_flat)
                            elif isinstance(recomen_from_flat, str):
                                recomen_from_flat = recomen_from_flat.strip("\"\'")
                                recomen_from_flat = f"\"{recomen_from_flat}\""
                            origin_row = key + ": " + value.strip()
                            update_row = key + ": " + recomen_from_flat
                            out_row = out_row.replace(origin_row, update_row)
                            out_rows[row_num] = out_row
                            print(f"\t\tUpdating row:\n\t\t\t{origin_row}\n\t\t\t\t->\n\t\t\t{update_row}")
                    elif priority == "FLAT.default_per_flavor":
                        print(f"\t{priority} available: {defaults_from_flat}")
                        # default_small_system_profile
                        # default_standard_system_profile
                        # default_large_system_profile
                        if not taken and defaults_from_flat is not None:
                            if system_size_mapped is None:
                                ...
                                sys.exit(1)
                            defaults_from_flat_chosen = defaults_from_flat[system_size_mapped] if system_size_mapped in defaults_from_flat else None
                            if isinstance(defaults_from_flat_chosen, bool):
                                if defaults_from_flat_chosen:
                                    defaults_from_flat_chosen = "true"
                                else:
                                    defaults_from_flat_chosen = "false"
                            elif isinstance(defaults_from_flat_chosen, int):
                                defaults_from_flat_chosen = str(defaults_from_flat_chosen)
                            elif isinstance(defaults_from_flat_chosen, str):
                                defaults_from_flat_chosen = defaults_from_flat_chosen.strip("\"\'")
                                defaults_from_flat_chosen = f"\"{defaults_from_flat_chosen}\""
                            print(f"\t{priority} chosen: {defaults_from_flat_chosen}")
                            if defaults_from_flat_chosen is not None:
                                taken = True
                                origin_row = key + ": " + value.strip()
                                update_row = key + ": " + defaults_from_flat_chosen
                                out_row = out_row.replace(origin_row, update_row)
                                out_rows[row_num] = out_row
                                print(f"\t\tUpdating row:\n\t\t\t{origin_row}\n\t\t\t\t->\n\t\t\t{update_row}")
                    elif priority == "FLAT.default":
                        if isinstance(default_from_flat, bool):
                            if default_from_flat:
                                default_from_flat = "true"
                            else:
                                default_from_flat = "false"
                        elif isinstance(default_from_flat, int):
                            default_from_flat = str(default_from_flat)
                        elif isinstance(default_from_flat, str):
                            default_from_flat = default_from_flat.strip("\"\'")
                            default_from_flat = f"\"{default_from_flat}\""
                        print(f"\t{priority}: {default_from_flat}")
                        if not taken and default_from_flat is not None:
                            if isinstance(default_from_flat, str):
                                if default_from_flat in value:
                                    taken = True
                                    origin_row = key + ": " + value.strip()
                                    update_row = key + ": " + default_from_flat
                                    out_row = out_row.replace(origin_row, update_row)
                                    out_rows[row_num] = out_row
                                    print(f"\t\tUpdating row:\n\t\t\t{origin_row}\n\t\t\t\t->\n\t\t\t{update_row}")
                    elif priority == "FLAT.example":
                        print(f"\t{priority}: {example_from_flat}")
                        if isinstance(example_from_flat, bool):
                            if example_from_flat:
                                example_from_flat = "true"
                            else:
                                example_from_flat = "false"
                        elif isinstance(example_from_flat, int):
                            example_from_flat = str(example_from_flat)
                        elif isinstance(example_from_flat, str):
                            example_from_flat = example_from_flat.strip("\"\'")
                            example_from_flat = f"\"{example_from_flat}\""
                        if not taken and example_from_flat is not None:
                            taken = True
                            origin_row = key + ": " + value.strip()
                            update_row = key + ": " + example_from_flat
                            out_row = out_row.replace(origin_row, update_row)
                            out_rows[row_num] = out_row
                            print(f"\t\tUpdating row:\n\t\t\t{origin_row}\n\t\t\t\t->\n\t\t\t{update_row}")
    ...
    try:
        ...
        if correct_yaml is None:
            ...
            if isinstance(first_item, str) and ":" not in first_item and " " in first_item:
                ...
                return False, correct_yaml
            elif last_block.lstrip().startswith('- 127.0.0.'):
                print("--------------------------------------------------------------------------------------------------")
...
def preprocess_yaml_file2b(in_yaml_content):
    ...
    # in this function were hardcoded mappings,
    # so I cannot share it with LLM
...
def wrong_syntax():
    print(
        ...
        "              [ --flat $PATH_TO_INPUT_FLAT_JSON_FILE ] \\ \n"
        ...
        "\t$PATH_TO_INPUT_FLAT_JSON_FILE     -- is a path to FLAT JSON file from CPI or from Git (optional)\n"
...
if __name__ == "__main__":
    ...
    PATH_TO_INPUT_FLAT_JSON_FILE = None
    ...
    if len(sys.argv) == 2:
        ...
        if sys.argv[2] == "--flat":
            PATH_TO_INPUT_FLAT_JSON_FILE = sys.argv[3]
        ...
        if sys.argv[2] == "--flat":
            ...
            PATH_TO_INPUT_HELM_CHARTS_FOLDER = sys.argv[3]
            if sys.argv[4] == "--flat":
                PATH_TO_INPUT_FLAT_JSON_FILE = sys.argv[5]
            elif sys.argv[4] == "--flavor":
                SYSTEM_SIZE = sys.argv[5]
            elif sys.argv[4] == "--config":
                PATH_TO_CONFIG_YAML_FILE = sys.argv[5]
        ...
        if sys.argv[2] == "--flat":
            ...
            if sys.argv[4] == "--flat":
                ...
                SYSTEM_SIZE = sys.argv[5]
                if sys.argv[6] == "--flat":
                    PATH_TO_INPUT_FLAT_JSON_FILE = sys.argv[7]
                elif sys.argv[6] == "--config":
                    PATH_TO_CONFIG_YAML_FILE = sys.argv[7]
            elif sys.argv[4] == "--config":
                PATH_TO_CONFIG_YAML_FILE = sys.argv[5]
        ...
        if sys.argv[2] == "--flat":
            ...
            if sys.argv[4] == "--flat":
                ...
                if sys.argv[6] == "--flat":
                    ...
                    PATH_TO_CONFIG_YAML_FILE = sys.argv[7]
                    if sys.argv[8] == "--flat":
                        PATH_TO_INPUT_FLAT_JSON_FILE = sys.argv[9]
            elif sys.argv[4] == "--config":
                PATH_TO_CONFIG_YAML_FILE = sys.argv[5]
                if sys.argv[6] == "--flat":
                    PATH_TO_INPUT_FLAT_JSON_FILE = sys.argv[7]
    ...
    if PATH_TO_INPUT_FLAT_JSON_FILE:
        print(f"\t$PATH_TO_INPUT_FLAT_JSON_FILE = {PATH_TO_INPUT_FLAT_JSON_FILE}")
    ...
    priorities = [
        "FLAT.recommended_value",
        "FLAT.default_per_flavor",
        "FLAT.default",
    ...
    if PATH_TO_INPUT_FLAT_JSON_FILE:
        with open(PATH_TO_INPUT_FLAT_JSON_FILE, "r", encoding="UTF-8") as flat_json_file:
            flat_json = json.load(flat_json_file)
            for param in flat_json["parameters"]:
                ...
                map_path_flat[path] = param
        # print(f"FLAT:\n")
        # # display(map_path_flat.keys())
        # print_keys(map_path_flat)
        # print()
    ...
    for yaml_template_path in glob.glob(PATH_TO_INPUT_YAML_TEMPLATE_FILE, recursive=True):
                    ...
                    if yamllint_error.level == "error" and yamllint_error.rule != "line-length":
                        ...
                        if yamllint_error.rule is None:
                            ...
                            or "expected <block end>, but found '<block mapping start>'" in yamllint_error.message:
                                ...
                                if not updated_this:
                                    # more complex scenario, we need to check FLAT and/or HELM charts, for example:
                                    # aaa-abcd-def:
                                    #   aaa-xyz1-xyz2-xyz3-xxx-yyy:
                                    #     cassandra:
                                    #       jvmOptions:
                                    #         set:
                                    #           - "-Djdk.tls.acknowledgeCloseNotify=true"
                                    #         #List of hhh iii jjj. Specified by IP address or hostname.
                                    #         #E.g:
                                    #         #    - 127.0.0.1
                                    #         #    - 127.0.0.2
                                    #         #      ijkLmnOpq:  <- first problem is here
                                    #         #        - DID0
                                    #         #        - DAD1
                                    #         #    purrrrh:            <- second problem is here
                                    #         #      afwenf:
                                    #         #        noienfJjnfPjj:
                                    #         #          #list of afnewf wnefoiwnef ounfowienf
                                    #         #          xyz.fff.ggg.qw/fef-wdwd-dwef-gtt: "pirjgf"
                                    #         #      oeringeor: <- third doubt is here
                                    #         #      ferferfFnirfFoomm: Abcdef   <- 4th doubt is here
                                    #         #      rfenrforin:
                                    #         # A name referring to ...........................
                                    #         #        fonefoinerfefFIinfrFKknn: aaa-xyz1-xyz2-xyz3-xxx-yyy-rfrf-cql-weef-rgtg-tgtg/aaa-xyz1-xyz2-xyz3-xxx-yyy-external-cql-rgrg-otjgto-rformf-qkwnd
                                    #         # A name referring to .....................
                                    #     #        efrfrfJinginrmOiinfn: aaa-xyz1-xyz2-xyz3-xxx-yyy-efrf-cql-gttg-fef  <- 5th problem
                                    block_to_find_arr = []
                                    ...
                                    while len(parent_path_found) > 0:
                                        ...
                                        if tmp_path_found in map_path_flat:
                                            good_path_found = tmp_path_found
                                            break
                                        parent_path_found = parent_path_found.rsplit('/', 1)[0]
                                        # expexted_indent_level -= 2
                                    if good_path_found:
                                        print(f"DEBUG: FLAT parent_path_found = {parent_path_found}")
                                        print(f"DEBUG: FLAT good_path_found = {good_path_found}")
                                        pp(map_path_flat[good_path_found])
        ...
        if "{{" in in_yaml_content_uncom and "}}" in in_yaml_content_uncom:
            ...
            in_yaml_content_uncom_set = fix_values(in_yaml_content_uncom, map_path_flat, map_path_helm, SYSTEM_SIZE, priorities)
