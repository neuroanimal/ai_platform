        ...
        for line in self.template_model.lines:
            ...
            if line.classification == "INACTIVE_DATA":
                ...
                if line.structure_node:
                    ...
                    elif force_uncomment_level != -1:
                        # ODCOMENTOWUJEMY
                        line.raw_content = line.raw_content.replace('# ', '', 1).replace('#', '', 1)
                        uncommented_count += 1

                        # Jeśli to kontener/lista, odpalamy lawinę dla dzieci
                        if line.structure_node and line.structure_node.node_type in ['object', 'array']:
                            force_uncomment_level = line.indent_level
                        self.tracer.trace_decision(
                            step="uncomment",
                            reason=f"Found children's comment in StructureModel at path: {line.identified_path}"
                        )
                        continue
