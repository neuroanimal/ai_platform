...
class TemplateValidator:
    ...
    def get_repair_instructions(self, issues: list) -> list:
        """
        Zamienia surowe błędy lintera na instrukcje dla AI Repair Agent.
        """
        instructions = []
        for issue in issues:
            if "indentation" in issue['message'].lower():
                instructions.append({
                    "type": "INDENT",
                    "line": issue['line'],
                    "fix": "Adjust spaces according to parent context"
                })
                elif "missing" in issue['message'].lower():
                instructions.append({
                    "type": "SYNTAX",
                    "line": issue['line'],
                    "fix": "Check for missing colons or array dashes"
                })
        return instructions
