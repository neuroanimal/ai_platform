...
class JSONModule:
    ...
    def read_parameters(self, path: str) -> list:
        """Wyciąga listę parametrów z płaskiego JSONa."""
        data = self.read_file(path)
        if isinstance(data, dict) and "parameters" in data:
            self.tracer.info(f"Wczytano {len(data['parameters'])} definicji parametrów z JSON.")
