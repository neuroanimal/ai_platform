...
class StructureNode:
    ...
    def __init__(self, name: str, node_type: str = "key", depth: int = 0):
        ...
        self.metadata = {
            "source": None,          # 'helm', 'json_params'  # helm, flat_json, or both
...
class StructureModel:
    ...
    def _determine_node_type_from_json(self, token: str, is_leaf: bool, param_format: str = None) -> str:
        if token == "[N]": return "array"
        if is_leaf:
            # Mapowanie formatów z JSON na nasze typy
            mapping = {
                "string": "string",
                "boolean": "boolean",
                "integer": "integer",
            ...
            return mapping.get(param_format, "string")
        return "object" # Jeśli ma dzieci i nie jest listą, to kontener
    ...
    def ingest_json_parameters(self, parameters: list):
        ...
        for param in parameters:
            ...
            for i, token in enumerate(tokens):
                ...
                # Jeśli węzeł nie istnieje, tworzymy go
                if token not in current.children:
                    n_type = self._determine_node_type_from_json(token, is_leaf, param.get("format"))
                    node = StructureNode(token, n_type, i)
                ...
                # Ustawiamy źródło i pochodzenie dla każdego przodka
                if current.metadata["source"] is None:
                    current.metadata["source"] = "json_params"
                    current.metadata["origin"] = "path_structure"
    ...
    def _tokenize_complex_path_new_later(self, path: str) -> list:
        ...
        # 3. Tymczasowo zamieniamy kropki WEWNĄTRZ nawiasów na unikalny marker
    ...
    def resolve_path_context(self, raw_path: str) -> Optional[StructureNode]:
        """
        Przeszukuje model, próbując dopasować ścieżkę z szablonu.
        ...
        Jest odporny na brakujące/nadmiarowe markery list [N].
        """
        ...
        """
        for i, token in enumerate(tokens):
            # 1. Próba dopasowania bezpośredniego
            if token in current.children:
                current = current.children[token]

            # 2. Próba dopasowania przez marker listy [N]
            # Jeśli w YAML nie ma [N], ale w modelu struktury jest lista
            elif "[N]" in current.children:
                # Sprawdzamy, czy nasze 'token' pasuje do dzieci tego [N]
                list_node = current.children["[N]"]
                if token in list_node.children:
                    current = list_node.children[token]
                else:
                    # Jeśli to sam marker [N], po prostu idziemy głębiej
                    current = list_node

            # 3. Próba dopasowania przez węzeł dynamiczny {{...}}
            else:
                dynamic_node = next((n for n in current.children.values() if n.node_type == "dynamic"), None)
                if dynamic_node:
                    current = dynamic_node
                else:
                    return None # Ścieżka przerwana
