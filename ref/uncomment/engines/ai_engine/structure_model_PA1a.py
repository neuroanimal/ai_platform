...
class StructureModel:
    ...
    def _ingest_dict(self, data: Any, parent_node: StructureNode = None, depth: int = 0):
        """Rekurencyjnie przetwarza zagniezdzony slownik z Helma."""
        if parent_node is None: parent_node = self.root

        if isinstance(data, dict):
            for key, value in data.items():
                node = self._get_or_create_node(parent_node, key, "key", depth)
                node.metadata["source"] = "helm"
                node.metadata["data_type"] = type(value).__name__
                self._ingest_dict(value, node, depth + 1)
        elif isinstance(data, list):
            # Obsluga list w Helm - tworzymy wezel specjalny dla elementów listy
            node = self._get_or_create_node(parent_node, "[N]", "index", depth)
            node.metadata["is_array_element"] = True
            if data:
                self._ingest_dict(data[0], node, depth + 1)
