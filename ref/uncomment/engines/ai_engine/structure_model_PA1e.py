            ...
            for i, token in enumerate(tokens):
                ...
                if i == len(tokens) - 1:
                    current.metadata.update({
                        "source": "json",
                        "mandatory": param.get("mandatory"),
                        "format": param.get("format"),
                        "description": f"Status: {param.get('status')}, Release: {param.get('release')}"
                    })

    def _tokenize_complex_path(self, path: str) -> list:
        """Rozbija ścieżkę na części, pilnując, by nie rozbić placeholderów {{...}}."""
        # Najpierw zamieniamy [2] na [N] dla uproszczenia modelu
